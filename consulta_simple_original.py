...
def resolver_captcha(self):
    ...
    lambda x: 0 if x < 128 else 255,
     '1')  # Binarizar
        ...
        logger.warning(
                    "⚠️  CAPTCHA no legible, usando valor por defecto")
        ...
        logger.warning(
                "⚠️  pytesseract no instalado, usando CAPTCHA simulado")
            return "1234"
        except Exception as e:
        logger.warning(
    f"⚠️  Error resolviendo CAPTCHA: {e}, usando simulado")
            return "TEST"
...
def _extraer_datos_pdf(self, pdf_path: str) -> Dict:
        ...
        logger.warning(
                "⚠️  PyPDF2 no instalado, no se pueden extraer datos")
...
Ejemplos REALES:
  ...
  %(prog)s --test-paralelo  # Prueba con 15 documentos
    ...
    '-c',
    type=str,
     help='Número de cédula a consultar')
    parser.add_argument(
    '--fecha',
    '-f',
    type=str,
     help='Fecha expedición (DD/MM/YYYY)')
    parser.add_argument(
    '--archivo',
    '-a',
    type=str,
     help='Archivo CSV con cédulas')
    parser.add_argument(
    '--test-paralelo',
    action='store_true',
    ...
    f"  ❌ Error: {
        ...
        if Path("main_final.py").exists():
            ...
            print(
                "ℹ️  Ejecuta: python -m pytest tests/parallel/test_concurrent_queries.py -v")
